#!/usr/bin/env python3
"""
Qwen3.5-Omni 视频场景分析 v3
按场景变化分段输出，每段含 time_start/time_end + detailed_analysis
"""
...
SYSTEM_PROMPT = """你是一个专业的视频场景分析师。你的任务是将视频按场景变化分成多个段落，对每段进行详细分析。

要求：
1. 根据画面内容、镜头切换、动作变化等将视频分成自然场景段落
2. 每个段落必须包含 time_start 和 time_end（精确到0.1秒）
3. 物体描述要具体：颜色/材质/位置、动作细节、污渍的形态位置、使用的工具
4. 文本元素：画面中出现的任何文字（产品名称、标签、说明文字、品牌名）
5. suitable_for：判断这段画面最适合广告的哪个阶段，可以选多个
6. detailed_analysis：对整个段落的综合分析，包括画面构图、视觉重点、画面变化过程、广告价值评估
7. 输出严格的JSON格式，不要添加任何非JSON内容"""

def make_user_prompt(duration):
    prompt = f"""视频时长约{duration}秒。请将视频按场景变化分成自然段落，每个段落输出一个JSON对象，包含以下字段：
- time_start: 段落开始时间（数字，秒）
- time_end: 段落结束时间（数字，秒）
...
- detailed_analysis: 对这个段落的综合分析（500字以内），包括：画面构图和视觉重点、画面变化过程、与前面段落的逻辑关系、这段画面在广告中的价值和作用

输出格式示例：
[{{"time_start":0,"time_end":2.5,"scene":"布满黄褐色油垢的不锈钢抽油烟机滤网特写","objects":["不锈钢抽油烟机滤网","黄褐色粘稠油污"],"text_elements":"重油污清洗剂","action":"画面静止展示严重积油状态，油污覆盖滤网表面并滴落。","lighting":"明亮的室内顶光，金属表面有反光","camera":"固定俯拍近景特写","mood":"痛点展示（脏乱）","suitable_for":["痛点"],"detailed_analysis":"画面以俯拍特写展示抽油烟机滤网上的严重油污，黄褐色粘稠油污覆盖整个滤网表面，部分油污已呈滴落状。金属反光与暗色油污形成强烈对比，视觉冲击力强。此段作为广告开场，有效建立用户对油污问题的共鸣和痛点感知。"}}]
...
def call_omni(video_b64, prompt):
    ...
    messages = [
        ...
        {"role": "user", "content": [{"video": video_uri}, {"text": prompt}]}
    ]
...
def analyse(filename, name, duration):
    ...
    prompt = make_user_prompt(duration)
    ...
    print(f"  ✅ 保存 {out_file} ({len(data)}个场景段)", flush=True)
    return True
